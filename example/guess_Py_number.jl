...
function guess_Pythagoras_number(
    ...
    for Py_num in lower_bound:upper_bound
        ...
        for idx_run = 1:num_run
            ...
            time_start = time()
            vec_sol, val_res, flag_conv = call_NLopt(num_square, target_sos, coord_ring, tuple_linear_forms=tuple_start, num_max_eval=coord_ring.dim1*REL_MAX_ITER, print_level=0) 
            time_end = time()
        ...
        if flag_success
            printfmtln("\n\n\nThe Pythagoras number for {}-dimensional degree-{} variety is conjectured to be {}.",
                      dim, deg, Py_num)
            return Py_num
        end
    end
