...
function construct_proj_excursion(
        mat_diff::Matrix{Float64}, 
        map_quotient::AbstractMatrix{Float64}
    )
    # get the projected difference matrix as a vector
    vec_diff = map_quotient * convert_sym_to_vec(mat_diff)
    # calculate the projection matrix
    map_quotient_excursion = SparseArrays.sparse(LinearAlgebra.I - vec_diff * vec_diff' ./ LinearAlgebra.norm(vec_diff)^2)
